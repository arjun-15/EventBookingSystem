...
import { ArrowLeft, CheckCircle, XCircle, Ban, Settings, IndianRupee, Users as UsersIcon, Calendar, Search, Star, Loader2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext.jsx';
import { eventService } from '../api/eventService';
import { analyticsService } from '../api/analyticsService';
import { authService } from '../api/authService';
import { mockReviews, mockCommissionSettings } from '../data/mockData.js';
...
export const AdminDashboard = ({ onNavigate }) => {
    ...
    const [allStats, setAllStats] = useState({ totalEvents: 0, totalUsers: 0, totalBookings: 0, totalRevenue: 0, adminCommission: 0 });
    const [loading, setLoading] = useState(true);

    const fetchData = async () => {
        setLoading(true);
        try {
            const [eventData, statsData, userData] = await Promise.all([
                eventService.getAllEvents(),
                analyticsService.getSystemStats(),
                // authService.getAllUsers() // Assuming this exists or using statsData.totalUsers
            ]);
            setEvents(eventData || []);
    ...
    const totalRevenue = allStats.totalRevenue || 0;
    const adminRevenue = allStats.adminCommission || 0;
    const statsUserCount = allStats.totalUsers || 0;
                            ...
                            <div className="premium-card p-6 shadow-none">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-2xl">{allStats.totalEvents}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="premium-card p-6 shadow-none">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-2xl">{statsUserCount}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="premium-card p-6 shadow-none">
