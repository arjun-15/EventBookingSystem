    ...
    return (
        <div className="min-h-screen bg-transparent">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 pb-12">
                ...
                {view === 'email' && <CommunicationTool events={events} onBack={() => setView('overview')} />}
            </div>
        </div>
    );
};
...
const CreateEventForm = ({ onComplete }) => {
    ...
    const handleSubmit = (e) => {
        ...
        try {
            const stored = localStorage.getItem('persistent_events');
            const currentEvents = stored ? JSON.parse(stored) : mockEvents;
            const updatedEvents = Array.isArray(currentEvents) ? [...currentEvents, newEvent] : [...mockEvents, newEvent];
            localStorage.setItem('persistent_events', JSON.stringify(updatedEvents));
...
const CommunicationTool = ({ events, onBack }) => {
    ...
    const handleSend = (e) => {
        ...
        setSending(true);
        setTimeout(() => {
            toast.success(`Broadcasting email to all attendees of selected event...`);
            setSending(false);
            setSubject('');
            setMessage('');
        }, 2000);
